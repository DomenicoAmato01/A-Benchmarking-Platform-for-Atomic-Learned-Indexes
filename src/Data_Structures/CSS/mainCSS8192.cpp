...
void run_baseline_experiments(std::vector<K> &keys, std::vector<T> &dataset,char **argv,int argc) {
    ...
    std::cout << std::endl << "# CSS-tree 8192" << std::endl;
    std::cout << "page size (bytes),height,size (bytes),query time (ns)" << std::endl;
    static_for_pow_two<8192, 8192>([&](auto page_size) {
...
int main(int argc, char **argv) {
	...
	if(argc<2) 
    { 
        printf("\nProgram usage: ./CSS8192 PAGE_SIZE PATH_TO_DATASET_FILE [PATH_TO_QUERY_FILE]"); 
	return 1;
	}
